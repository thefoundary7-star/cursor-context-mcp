...
try {
    ...
    # Create index.js with an overly long function
...
"@ -Encoding UTF8
    ...
    # Create math.py
    Set-Content -Path "src\math.py" -Value @"
...
"@ -Encoding UTF8
    ...
    # Create util.ts
    Set-Content -Path "src\util.ts" -Value @"
...
"@ -Encoding UTF8
    ...
    npm install express lodash | Out-Null
    npm install --save-dev jest "@types/jest" typescript ts-node | Out-Null

    # 4. Create tests
    Write-Host "🧪 Creating test files..." -ForegroundColor Yellow
    New-Item -ItemType Directory -Name "tests" -Force | Out-Null

    Set-Content -Path "tests\sample.test.js" -Value @"
...
"@ -Encoding UTF8
    ...
    npm pkg set scripts.test="jest" | Out-Null
    npm pkg set scripts.start="node src/index.js" | Out-Null
    npm pkg set scripts.dev="node src/index.js" | Out-Null
